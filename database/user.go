...
func (uq *UserQuery) GetByMXID(userID id.UserID) *User {
	...
	return uq.New().Scan(uq.db.QueryRow(context.Background(), query, userID))
}
...
func (uq *UserQuery) GetByID(id string) *User {
	...
	return uq.New().Scan(uq.db.QueryRow(context.Background(), query, id))
}
...
func (uq *UserQuery) GetAllWithToken() []*User {
	...
	`
	rows, err := uq.db.Query(context.Background(), query)
	if err != nil || rows == nil {
		return nil
	}
...
func (u *User) Insert() {
	...
	_, err := u.db.Exec(context.Background(), query, u.MXID, strPtr(u.DiscordID), strPtr(u.DiscordToken), strPtr(string(u.ManagementRoom)), strPtr(string(u.SpaceRoom)), strPtr(string(u.DMSpaceRoom)), u.ReadStateVersion, JSONPtr(u.HeartbeatSession))
	if err != nil {
...
func (u *User) Update() {
	...
	_, err := u.db.Exec(context.Background(), query, strPtr(u.DiscordID), strPtr(u.DiscordToken), strPtr(string(u.ManagementRoom)), strPtr(string(u.SpaceRoom)), strPtr(string(u.DMSpaceRoom)), u.ReadStateVersion, JSONPtr(u.HeartbeatSession), u.MXID)
	if err != nil {
