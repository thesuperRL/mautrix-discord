...
func (helper *CryptoHelper) Init() error {
	...
	err := helper.store.DB.Upgrade(context.Background())
	if err != nil {
...
func (helper *CryptoHelper) resyncEncryptionInfo() {
	...
	rows, err := helper.bridge.DB.Query(context.Background(), `SELECT room_id FROM mx_room_state WHERE encryption='{"resync":true}'`)
	if err != nil {
	...
	if len(roomIDs) > 0 {
		...
		for _, roomID := range roomIDs {
			...
			if err != nil {
				...
				_, err = helper.bridge.DB.Exec(context.Background(), `
...
func (helper *CryptoHelper) clearDatabase() {
	_, err := helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_account")
	if err != nil {
		helper.log.Warn().Err(err).Msg("Failed to clear crypto_account table")
	}
	_, err = helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_olm_session")
	if err != nil {
		helper.log.Warn().Err(err).Msg("Failed to clear crypto_olm_session table")
	}
	_, err = helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_megolm_outbound_session")
	if err != nil {
		helper.log.Warn().Err(err).Msg("Failed to clear crypto_megolm_outbound_session table")
	}
	//_, _ = helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_device")
	//_, _ = helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_tracked_user")
	//_, _ = helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_cross_signing_keys")
	//_, _ = helper.store.DB.Exec(context.Background(), "DELETE FROM crypto_cross_signing_signatures")
}
