...
func (mx *MatrixHandler) HandleEncryption(_ context.Context, evt *event.Event) {
...
func (mx *MatrixHandler) joinAndCheckMembers(ctx context.Context, evt *event.Event, intent *appservice.IntentAPI) *mautrix.RespJoinedMembers {
	log := zerolog.Ctx(ctx)
	resp, err := intent.JoinRoomByID(context.Background(), evt.RoomID)
	if err != nil {
	...
	if err != nil {
		...
		_, _ = intent.LeaveRoom(context.Background(), resp.RoomID)
		return nil
	}
...
func (mx *MatrixHandler) sendNoticeWithMarkdown(roomID id.RoomID, message string) (*mautrix.RespSendEvent, error) {
	...
	return intent.SendMessageEvent(context.Background(), roomID, event.EventMessage, content)
}
...
func (mx *MatrixHandler) HandleBotInvite(ctx context.Context, evt *event.Event) {
	...
	if user.GetPermissionLevel() < bridgeconfig.PermissionLevelUser {
		_, _ = intent.SendNotice(context.Background(), evt.RoomID, "You are not whitelisted to use this bridge.\n"+
			"If you're the owner of this bridge, see the bridge.permissions section in your config file.")
		_, _ = intent.LeaveRoom(context.Background(), evt.RoomID)
		return
	}
	...
	if len(members.Joined) == 2 && (len(user.GetManagementRoomID()) == 0 || evt.Content.AsMember().IsDirect) {
		...
		_, _ = intent.SendNotice(context.Background(), user.GetManagementRoomID(), "This room has been registered as your bridge management/status room.")
...
func (mx *MatrixHandler) HandleGhostInvite(ctx context.Context, evt *event.Event, inviter User, ghost Ghost) {
	...
	if inviter.GetPermissionLevel() < bridgeconfig.PermissionLevelUser {
		...
		_, err := intent.LeaveRoom(context.Background(), evt.RoomID, &mautrix.ReqLeave{
	...
	if err := intent.StateEvent(context.Background(), evt.RoomID, event.StateCreate, "", &createEvent); err != nil {
	...
	if !hasBridgeBot && !hasOtherUsers && evt.Content.AsMember().IsDirect {
		...
		_, _ = intent.SendNotice(context.Background(), evt.RoomID, "Please invite the bridge bot first if you want to bridge to a remote chat.")
		_, _ = intent.LeaveRoom(context.Background(), evt.RoomID)
	} else {
		_, _ = intent.SendNotice(context.Background(), evt.RoomID, "This puppet will remain inactive until this room is bridged to a remote chat.")
	}
}

func (mx *MatrixHandler) HandleMembership(_ context.Context, evt *event.Event) {
...
func (mx *MatrixHandler) HandleRoomMetadata(_ context.Context, evt *event.Event) {
	defer mx.TrackEventDuration(evt.Type)()
	if mx.shouldIgnoreEvent(evt) {
		return
	}
...
func (mx *MatrixHandler) sendCryptoStatusError(ctx context.Context, evt *event.Event, editEvent id.EventID, err error, retryCount int, isFinal bool) id.EventID {
	...
	if mx.bridge.Config.Bridge.EnableMessageStatusEvents() {
		...
		_, sendErr := mx.bridge.Bot.SendMessageEvent(context.Background(), evt.RoomID, event.BeeperMessageStatus, statusEvent)
		if sendErr != nil {
	...
	if mx.bridge.Config.Bridge.EnableMessageErrorNotices() {
		...
		resp, sendErr := mx.bridge.Bot.SendMessageEvent(context.Background(), evt.RoomID, event.EventMessage, &update)
		if sendErr != nil {
...
func (mx *MatrixHandler) postDecrypt(ctx context.Context, original, decrypted *event.Event, retryCount int, errorEventID id.EventID, duration time.Duration) {
	...
	mx.bridge.EventProcessor.Dispatch(context.Background(), decrypted)
	if errorEventID != "" {
		_, _ = mx.bridge.Bot.RedactEvent(context.Background(), decrypted.RoomID, errorEventID)
	}
}

func (mx *MatrixHandler) HandleEncrypted(_ context.Context, evt *event.Event) {
	defer mx.TrackEventDuration(evt.Type)()
	if mx.shouldIgnoreEvent(evt) {
		return
	}
...
func (mx *MatrixHandler) HandleMessage(_ context.Context, evt *event.Event) {
	defer mx.TrackEventDuration(evt.Type)()
	if mx.shouldIgnoreEvent(evt) {
		return
