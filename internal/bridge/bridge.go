...
func (br *Bridge) ensureConnection() {
	for {
		...
		if err != nil {
			if errors.Is(err, mautrix.MForbidden) {
				...
				err = br.Bot.EnsureRegistered(context.Background())
				if err != nil {
	...
	resp, err := br.Bot.Whoami(context.Background())
	if err != nil {
...
func (br *Bridge) UpdateBotProfile() {
	...
	if botConfig.Avatar == "remove" {
		err = br.Bot.SetAvatarURL(context.Background(), mxc)
	} else if !botConfig.ParsedAvatar.IsEmpty() {
		err = br.Bot.SetAvatarURL(context.Background(), botConfig.ParsedAvatar)
	}
	if err != nil {
	...
	if botConfig.Displayname == "remove" {
		err = br.Bot.SetDisplayName(context.Background(), "")
	} else if len(botConfig.Displayname) > 0 {
		err = br.Bot.SetDisplayName(context.Background(), botConfig.Displayname)
	}
	if err != nil {
...
func (br *Bridge) start() {
	...
	err := br.DB.Upgrade(context.Background())
	if err != nil {
		br.LogDBUpgradeErrorAndExit("main", err)
	} else if err = br.StateStore.Upgrade(context.Background()); err != nil {
	...
	br.EventProcessor.Start(context.Background())
